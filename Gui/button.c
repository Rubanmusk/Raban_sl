...
_btn_obj * btn_creat(u16 left,u16 top,u16 width,u16 height,u8 id,u8 type)
{
	...
	}else btn_crt->bkctbl=NULL;//对标准按钮
    return btn_crt;
}
//删除按钮
//btn_del:要删除的按键
...
void btn_draw_picbtn(_btn_obj * btnx)
{
	u8 sta;
 	 ...
 	 switch(sta)
	 {
		case BTN_RELEASE://松开
		case BTN_INACTIVE://未激活	
			gui_draw_color_bmp(btnx->left,btnx->top,btnx->width,btnx->height,btnx->picbtnpathu);
 			break;
		case BTN_PRESS://按下,当存在按下图片时,显示按下的图片.当不存在时,在松开图片上做alphablending
					
	  if(btnx->picbtnpathd!=NULL)
	    {
 			gui_draw_color_bmp(btnx->left,btnx->top,btnx->width,btnx->height,btnx->picbtnpathd); 			
 		  }		   
			break;
	  }	
 	//显示caption
	if(sta==0)gui_show_strmid(btnx->left+btnx->caption_left,btnx->top+btnx->caption_top,btnx->width-btnx->caption_left,btnx->height-btnx->caption_top,btnx->bcfucolor,btnx->font,btnx->caption,1);
	else gui_show_strmid(btnx->left+btnx->caption_left+1,btnx->top+btnx->caption_top+1,btnx->width-btnx->caption_left,btnx->height-btnx->caption_top,btnx->bcfdcolor,btnx->font,btnx->caption,1);		 	
}  
//画圆角按钮
//btnx:按钮
