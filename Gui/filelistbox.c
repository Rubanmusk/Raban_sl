...
//All rights reserved									  
//////////////////////////////////////////////////////////////////////////////////
//extern const unsigned char GCODEFILE_ICON[126];//20150909
...
//filelistbox:listbox指针
//in_key:输入按键指针
//返回值:返回被双击的动作 	0x01 进入下一个目录 0x02 返回上一级目录 0x3 目标文件   0xff 无双击动作		   
				...
				}else //按键松开了,或者滑出了
				{
					...
					}else if(filelistbox->sta&(1<<6))//单点按下
					{
						...
						if((filelistbox->scbv->topitem+tempindex)==filelistbox->selindex)
						{
							...
							reg=filelistbox_check_filelist(filelistbox);//文件夹 	0x01 进入下一个目录 0x02 返回上一级目录 0x3 目标文件
						...
						{	  
							if((filelistbox->selindex<(filelistbox->scbv->topitem+filelistbox->scbv->itemsperpage))&&(filelistbox->selindex>=filelistbox->scbv->topitem))//老的selindex在屏幕范围内
							{
								...
								if(filelistx->type==1)//文件夹
								  gui_draw_single_color_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,30,30,SYSTEM_FOLDER,ORANGECOL,WHITE);	//20150909
                else//其他
									gui_draw_single_color_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,30,30,SYSTEM_GCODEFILE,ORANGECOL,WHITE);	//20150909
 
								gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,gui_phy.lcdheight,0,filelistbox->lncolor,filelistbox->font,filelistx->name,1);//显示名字
							}
							...
							//gui_draw_single_color_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,30,30,SYSTEM_GCODEFILE,BLACK,WHITE);	//20150909

							gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,gui_phy.lcdheight,0,filelistbox->lnselcolor,filelistbox->font,filelistx->name,1);//显示名字
							...
							if(filelistbox->selindex>=filelistbox->scbv->topitem&&filelistbox->selindex<(filelistbox->scbv->topitem+filelistbox->scbv->itemsperpage))//选中条目在当前的显示范围内
	 						{	 
								...
								if(i<filelistbox->namelen)//小于文字长度,就是不能够一次显示的
								{
									...
									gui_fill_rectangle(filelistbox->left+FLBOX_ICO_WIDTH,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT,endx-filelistbox->left+1-FLBOX_ICO_WIDTH,FLBOX_ITEM_HEIGHT,filelistbox->lnselbkcolor);//填充选中后的底色
		 							gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,gui_phy.lcdheight,filelistbox->curnamepos,filelistbox->lnselcolor,filelistbox->font,filelistbox->fname,1);//显示名字
...
void filelistbox_draw_list(_filelistbox_obj *filelistbox)
{
	...
	if(filelistbox->scbv->totalitems>filelistbox->scbv->itemsperpage)
	{
 	...
 	gui_fill_rectangle(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT,endx-filelistbox->left+1,filelistbox->height-FLBOX_PATH_HEIGHT-FLBOX_FINFO_HEIGHT,filelistbox->lbkcolor);	
	...
	{	
		...
		if(filelistx->type==1)//文件夹
		  ...
		  gui_draw_single_color_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,30,30,SYSTEM_GCODEFILE,ORANGECOL,WHITE);
	
		gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,gui_phy.lcdheight,0,tempcolor,filelistbox->font,filelistx->name,1);//显示名字
	...
	if(plen)//需要显示
	{
		if(plen*(FLBOX_PTHINF_FONT/2)>(filelistbox->width-16-7))
		{
			...
			while(plen*(FLBOX_PTHINF_FONT/2)>(filelistbox->width-offset-16-7))
			{
 			...
 			gui_show_ptstr(filelistbox->left+16+7,filelistbox->top+(FLBOX_PATH_HEIGHT-FLBOX_PTHINF_FONT)/2,filelistbox->width+filelistbox->left,gui_phy.lcdheight,0,filelistbox->lncolor,FLBOX_PTHINF_FONT,"...",1);//显示3点
		}   	
		gui_show_ptstr(filelistbox->left+offset+16+7,filelistbox->top+(FLBOX_PATH_HEIGHT-FLBOX_PTHINF_FONT)/2,filelistbox->width+filelistbox->left,gui_phy.lcdheight,0,filelistbox->lncolor,FLBOX_PTHINF_FONT,path,1);//显示路径
	}
}	 
...
void filelistbox_show_info(_filelistbox_obj *filelistbox)
{
	...
	gui_show_ptstr(filelistbox->left,filelistbox->top+filelistbox->height-FLBOX_FINFO_HEIGHT+(FLBOX_FINFO_HEIGHT-FLBOX_PTHINF_FONT)/2,filelistbox->width+filelistbox->left,gui_phy.lcdheight,0,filelistbox->lncolor,FLBOX_PTHINF_FONT,temp,1);//显示路径
	gui_memin_free(temp);	
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
	...
	u8 mark=0;//0,查找文件夹;1,查找目标文件;控制是否跳过文件夹
...
//返回值：NULL: 申请内存失败 0x01:进入新目录，0x02：返回上一层目 0x03：目标文件
u8 filelistbox_check_filelist(_filelistbox_obj *filelistbox)
{
	u8 *str;
	u8 *oldpath;
	u16 temp;
	...
	{   
		...
		{	
			...
			if(filelistbox->path==NULL)return NULL;//申请内存失败.
 			strcpy((char *)filelistbox->path,(const char*)str);//拷贝str到path里面
		}else //是文件夹 通过当前路径加上文件名组合成新的访问路径
		{
			if(strcmp((const char*)filelistx->name,".."))//不是..文件名
			{
				...
				if(oldpath==NULL)return NULL;//申请内存失败
