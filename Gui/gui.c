...
{
	"确定","確定","OK","犬牢","Sí","Ja","Да","Oui","Sim","Sì",
};
//选项
...
{
	"取消","取消","CANCEL","秒家","NO","Nein","Нет","Aucune","NO","NO",
};
//退出
...
//	"退出","退出","QUIT",		    
//};
const u8* GUI_FINISH_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"完成","完成","FINISH","肯己","Terminar","FINISH","FINISH","FINISH","FINISH","FINISH",
};
const u8* GUI_CONTINUE_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"续打","續打","CONTINUE","拌加","Continúa.","CONTINUE","CONTINUE","CONTINUE","CONTINUE","CONTINUE",
};
const u8* GUI_AGAIN_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"重打","重打","AGAIN","促矫矫累","Aquí vamos.","AGAIN","AGAIN","AGAIN","AGAIN","AGAIN",
};
...
void gui_draw_vline(u16 x0,u16 y0,u16 len,u16 color)
{
	...
	gui_phy.fill(x0,y0,1,len,color);
}
//画水平线
//x0,y0:坐标
//len:线长度
...
{
	if((len==0)||(x0>gui_phy.lcdwidth)||(y0>gui_phy.lcdheight))return;
	gui_phy.fill(x0,y0,len,1,color);	
}
//填充一个色块.
...
void gui_draw_smooth_rectangle(u16 x,u16 y,u16 width,u16 height,u32 srgb,u32 ergb)
{
	...
	LCD_Set_Window(x,y,width,height); //设置光标位置   
	LCD_WriteRAM_Prepare();	//开始写入GRAM
...
//20150909
/***********************************
	功能：读取Flash中的图标数据进行显示    最大256（u8）
	参数1：x，       图形起始点的x坐标
	参数2：y，       图形起始点的y坐标
	参数3：width      图形宽
	参数4：height      图形高
  参数5：index   图标在外部Flash中的偏离ICON_START_ADDR的偏移地址的指针 
	参数6：pen_color，图形颜色
	参数7：back_color，图形背景颜色
  返回：   无
************************************/
void gui_draw_single_color_icos(u16 x,u16 y,u16 width,u16 height,u8 *index,u16 pen_color,u16 back_color)
{
	u16 i,j;      
	u16 group=0;
	u32 sum=0;
    ...
    SPI_Flash_Read(icon_buf,ICON_START_ADDR+*(u32*)index,6);
	  sum=ICON_START_ADDR+*(u32*)index+6;
			if((width==(icon_buf[2]+(icon_buf[3]<<8)))&&(height==(icon_buf[4]+(icon_buf[5]<<8))))
			{
				...
				for(j=0;j<height;j++)	//列循环
					{
						for(i=0;i<width;i++)//写一行像素
							 {  	
								 group=i/8;
								 offset=i%8;
								 if(offset==0)								 
								 SPI_Flash_Read(icon_buf,sum+group,1);
									 if(icon_buf[0] &(0x80>>offset))
										 {	
											 color=pen_color;//
										 }		    
									 else
										 {	
											...
											//LCD_WR_DATA(color);//fsmc
										 LCD_WriteRAM(color);//mini版--spi
							 }
							 sum = sum+group+1;
					}
			}
...
//20160522
/***********************************
	功能：读取Flash中的BMP数据进行显示    最大356（u8）
	参数1：x，       图形起始点的x坐标
	参数2：y，       图形起始点的y坐标
	参数3：width      图形宽
	参数4：height      图形高
  ...
  返回：   无
************************************/
void gui_draw_color_bmp(u16 x,u16 y,u16 width,u16 height,u8 *index)
{	u32 i,j;      
	u16 color;
	u8 * icon_buf_0;
	u8 * icon_buf_1;
	u8 icon_buf_choice=0;	

	u32 num=width<<1;
	icon_buf_0=gui_memin_malloc(num);	//
	icon_buf_1=gui_memin_malloc(num);	//
  //Dlion MINI版使用SPI1
	if((icon_buf_0!=NULL)&(icon_buf_1!=NULL))		
	{	
			SPI_Flash_ReadAddr_start(THEME_ADDR+9+*(u32*)index);	
			SPI1_DR_DMA_Configuration((u32)icon_buf_0,8);//读取
			SPI1_DR_DMA_Enable(8); 
 			while(!DMA_GetFlagStatus(DMA1_FLAG_TC2));
			SPI1_DR_DMA_Disable();  
		//	printf("OK1 "); 
		//	printf(" [%x] [%x] [%x] [%x] " ,icon_buf_0[2],icon_buf_0[3],icon_buf_0[4],icon_buf_0[5]); 
			if((width==(icon_buf_0[2]+(icon_buf_0[3]<<8)))&&(height==(icon_buf_0[4]+(icon_buf_0[5]<<8))))
			{//	printf("OK2  "); 
				SPI1_DR_DMA_Configuration((u32)icon_buf_0,num);//预读取缓存0
				SPI1_DR_DMA_Enable(num);	
				
				LCD_Set_Window(x,y,width,height); //设置光标位置   
				LCD_WriteRAM_Prepare();	//开始写入GRAM	
				
				while(!DMA_GetFlagStatus(DMA1_FLAG_TC2));   //等待缓存0预读结束
// 			for(i=0;i<width;i++)//写一行像素
// 					{	color=icon_buf_0[i*2] + (icon_buf_0[i*2+1]<<8) ;
// 						printf("[0x%x]",color); 
// 						}	
				SPI1_DR_DMA_Disable()	 ;  
				SPI1_DR_DMA_Configuration((u32)icon_buf_1,num);//预读缓存1
				SPI1_DR_DMA_Enable(num);
				
				icon_buf_choice=0; //缓存0预读完成所以 选择缓存0 
				
				for(j=0;j<height;j++)	//列循环
				{
					for(i=0;i<width;i++)//写一行像素
					{  	
						if(	icon_buf_choice)  //显示预读完成的缓存组
							{		color=icon_buf_1[(i<<1)] + (icon_buf_1[(i<<1)+1]<<8) ;
							}else
							{		color=icon_buf_0[(i<<1)] + (icon_buf_0[(i<<1)+1]<<8) ;
							}
						//LCD_WR_DATA(color);	//显示一个点
							LCD_WriteRAM(color);
					 }	
							icon_buf_choice=~icon_buf_choice;  //切换另一缓存
							
							while(!DMA_GetFlagStatus(DMA1_FLAG_TC2));   //等待上一组缓存缓存结束
							
							SPI1_DR_DMA_Disable()	 ;  
							
							if(icon_buf_choice)
							{		SPI1_DR_DMA_Configuration((u32)icon_buf_0,num);//预读缓存1
							}else
							{	SPI1_DR_DMA_Configuration((u32)icon_buf_1,num);//预读缓存0
							}
	...
	SPI1_DR_DMA_Disable()	 ;  
 	SPI_Flash_ReadAddr_end();	
	gui_memin_free(icon_buf_0);
	gui_memin_free(icon_buf_1);
	LCD_DisplayOn();
			

} 
/***********************************
	功能：只显示在FLASH里面的背景图某个区域的图像
	参数1：x，      显示区始点的x坐标
	参数2：y，       显示区始点的y坐标
	参数3：width      显示区宽
	参数4：height      显示区高
  ...
  返回：   无
************************************/
void gui_draw_windows_color_bmp(u16 x,u16 y,u16 xx,u16 yy,u16 width,u16 height,u8 *index)
{	u32 i,j;      
	u16 color;
	u8 * icon_buf_0;
	u8 * icon_buf_1;
	...
	icon_buf_0=gui_memin_malloc(num);	//
	icon_buf_1=gui_memin_malloc(num);	//
	addr=THEME_ADDR+9+*(u32*)index+8+(lcddev.width<<1)*yy+(xx<<1);
	if((icon_buf_0!=NULL)&(icon_buf_1!=NULL))		
	{	  
			SPI_Flash_ReadAddr_start(addr);	
			SPI1_DR_DMA_Configuration((u32)icon_buf_0,num);//预读取缓存0
			SPI1_DR_DMA_Enable(num); 
...
//将背景填除显示区域，并在显示区居中显示字符串 用于实时更新内容的显示
//x1,y1,width,height:显示区域坐标.(相对图片)
///x0,y0字符串显示位置(相对坐标原点)
//color:文字颜色.
//size:文字大小
//str:字符串
//index:背景图片
void gui_background_show_strmid(u16 x0,u16 y0,u16 x1,u16 y1,u16 width,u16 height,u16 color,u8 size,u8 *str,u8 *index)
{
	u16 xoff=0,yoff=0;
	u16 strlenth;
	u16 strwidth;
	...
	gui_draw_windows_color_bmp(x0,y0,x1,y1,width,height,index);
  strlenth=strlen((const char*)str);	//得到字符串长度
	...
	gui_show_ptstr(x0+xoff,y0+yoff,x0+width-1,y0+height-1,0,color,size,str,1);  
}
...
{		 							   
	...
	gui_phy.fill(x0,y0,width,height,color);	
}
//画实心圆
//x0,y0:坐标
//r半径
...
void gui_show_strmid(u16 x,u16 y,u16 width,u16 height,u16 color,u8 size,u8 *str,u8 mode)
{
	u16 xoff=0,yoff=0;
	u16 strlenth;
	u16 strwidth;
