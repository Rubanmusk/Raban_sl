...
//创建进度条	(圆角矩形进度条只支持水平的进度条)
//top,left,width,heigh,r:坐标.及尺寸. 
//type:按键类型 
//[bit7]:方向,0,水平;1,垂直;
...
_progressbar_obj * progressbar_creat(u16 left,u16 top,u16 width,u16 height,u16 r,u8 type)
{
 	_progressbar_obj * progressbar_crt;
	if(width<2||height<2)return NULL;//尺寸不能太小
	if((r>(width/2))||(r>(height/2)))return NULL;//圆角尺寸太大
...
//		percnt=(100*prgbx->curpos)/prgbx->totallen;//得到百分数
		percnt=(float)prgbx->curpos*100/prgbx->totallen;//得到百分数jee201805
		if(prgbx->type&PRGB_TYPE_VER)//垂直的
		{
...
void progressbar_setpos(_progressbar_obj *prgbx)
{
	...
	if(prgbx->type&PRGB_TYPE_VER)//垂直进度条
	{
		...
		if(prgbx->curpos > prgbx->totallen) 
		  prgbx->curpos = prgbx->totallen;//范围限定	
	...
	{		 
	  ...
	  prgblen=((float)prgbx->curpos/prgbx->totallen)*prgblen;//计算当前进度条的长度
		...
		if(prgbx->r==0)	//矩形进度条
		{
		gui_fill_rectangle(x+prgblen,y,xlen-prgblen,ylen/2,prgbx->inbkcolora);//清空进度条
	 	gui_fill_rectangle(x+prgblen,y+ylen/2,xlen-prgblen,ylen-ylen/2,prgbx->inbkcolorb);//清空进度条
			
//			gui_fill_rectangle(x,y,xlen,ylen/2,prgbx->infcolora);//
//	  			gui_fill_rectangle(x,y+ylen/2,xlen,ylen-ylen/2,prgbx->infcolorb);
		}
		else   //圆角矩形进度条
		{ gui_draw_arcrectangle(x,y,xlen,ylen,r,1,prgbx->inbkcolora,prgbx->inbkcolorb);
		}
	}
...
//	prgblen=((float)prgbx->curpos/prgbx->totallen)*prgblen;//计算当前进度条的长度
	...
	}else//水平进度条 
	{
		...
		{	if(prgbx->r==0)	//矩形进度条
			...
			{  	if(xlen>0)
				{	if(xlen<2*r) xlen=2*r;
				 	gui_draw_arcrectangle(x,y,xlen,ylen,r,1,prgbx->infcolora,prgbx->infcolorb);
				}
			}
		}
		x+=xlen;
...
void progressbar_draw_progressbar(_progressbar_obj *prgbx)
{
	...
	{	gui_draw_arcrectangle(prgbx->left,prgbx->top,prgbx->width,prgbx->height,prgbx->r,0,prgbx->rimcolor,prgbx->rimcolor);
		progressbar_setpos(prgbx);
	} 
}
//进度条的钩子函数
...
void test_prgb(u16 x,u16 y,u16 width,u16 height,u8 type,u16 icur)
{
	...
	prgb=progressbar_creat(x,y,width,height,0,type);//创建进度条
