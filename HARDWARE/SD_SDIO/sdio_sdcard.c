...
{	
	...
	//SDIO IO口初始化
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD, ENABLE);	 //使能PC,PD端口时钟
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA2|RCC_AHBPeriph_SDIO, ENABLE);	 //使能DMA2时钟,使能SDIO时钟

	//PORTC 8~12 复用推挽输出  
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;	//PORTC复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOC, &GPIO_InitStructure); 
	
	//PORTD 2 复用推挽输出  
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;	//PORTD 2复用推挽输出  
 	GPIO_Init(GPIOD, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	//SD_CD---上拉输入
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	u8 sta=SD_OK;
	u8 n;
//	if(CardType!=SDIO_STD_CAPACITY_SD_CARD_V1_1)sector<<=9;//20160330
	sector<<=9;
