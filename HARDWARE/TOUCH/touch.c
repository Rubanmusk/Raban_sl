...
{
	TP_Init, //初始化触摸屏控制器
	TP_Scan, //扫描触摸屏.0,屏幕扫描;1,物理坐标;	 
	TP_Adjust, //触摸屏校准 	   
	0,					//当前坐标
	0, //电容屏有最多5组坐标,电阻屏则用x[0],y[0]代表:此次扫描时,触屏的坐标,用
								//x[4],y[4]存储第一次按下时的坐标. 
	0,					//笔的状态 
								//b7:按下1/松开0; 
	                            //b6:0,没有按键按下;1,有按键按下. 
...
{ 
   SPI_FLASH_CS=1;//mini
 	return SPI1_ReadWriteByte(num);//SPI3总线读写一个字节
} 
//SPI读数据 
//从触摸屏IC读取adc值
//CMD:指令
...
//MINI版触摸与flash共用spi1
//把读到的数据存到TP_DATAINFO[6]
//void TP_ReadDataInfo(void)
//{
//	SPI_Flash_Read((u8*)&TP_DATAINFO[0],SAVE_ADDR_BASE,4);	//保存x校正因素     
//	SPI_Flash_Read((u8*)&TP_DATAINFO[1],SAVE_ADDR_BASE+4,4);//保存y校正因素       
//	SPI_Flash_Read((u8*)&TP_DATAINFO[2],SAVE_ADDR_BASE+8,2);//保存x偏移量
//	SPI_Flash_Read((u8*)&TP_DATAINFO[3],SAVE_ADDR_BASE+10,2);//保存y偏移量
//	SPI_Flash_Read((u8*)&TP_DATAINFO[4],SAVE_ADDR_BASE+12,1);//保存触屏类型	
//	SPI_Flash_Read((u8*)&TP_DATAINFO[5],SAVE_ADDR_BASE+13,1);//校准标记
//}
//得到保存在EEPROM里面的校准值
//返回值：1，成功获取数据
//        0，获取失败，要重新校准
...
{	
	...
	SPI_Flash_Read((u8*)&tempfac,SAVE_ADDR_BASE+13,1);//读取标记字,看是否校准过！ 
...
{	  
	...
	LCD_ShowNum((lcddev.width-160)/2+24,140,x0,4,16);		//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24+80,140,y0,4,16);	//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24,160,x1,4,16);		//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24+80,160,y1,4,16);	//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24,180,x2,4,16);		//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24+80,180,y2,4,16);	//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24,200,x3,4,16);		//显示数值
	LCD_ShowNum((lcddev.width-160)/2+24+80,200,y3,4,16);	//显示数值
 	LCD_ShowNum((lcddev.width-160)/2+56,220,fac,4,16); 	//显示数值,该数值必须在95~105范围之内.
...
{								 
	...
	LCD_Clear(WHITE);//清屏 	   
	POINT_COLOR=BLACK;
	LCD_ShowString((lcddev.width-160)/2,40,160,100,16,(u8*)TP_REMIND_MSG_TBL);//显示提示信息
		...
		if((tp_dev.sta&0xc0)==TP_CATH_PRES)//按键按下了一次(此时按键松开了.)
		{
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					POINT_COLOR=BLUE;
					LCD_Clear(WHITE);//清屏
					LCD_ShowString((lcddev.width-184)/2,110,184,16,16,"Touch Screen Adjust OK!");//校正完成
					delay_ms(1000);
...
//	{ 
		GPIO_InitTypeDef  GPIO_InitStructure;
	 	...
	 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;	// PC.4端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //	上拉输入
	 	GPIO_Init(GPIOB, &GPIO_InitStructure);
		
	 	SPI1_Init();		   	//初始化TOUCH XPT2046 SPI接口
		//IIC_Init();;				//初始化24CXX IIC 接口
		//mini版
	  //TP_ReadDataInfo();//jee
