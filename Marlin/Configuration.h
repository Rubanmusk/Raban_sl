...
//// The following define selects which electronics board you have. Please choose the one that matches your setup//Marlin固件支持的主控板清单
...
#define DELTA_SEGMENTS_PER_SECOND 160//减小这个值会减小卡顿现象


// Center-to-center distance of the holes in the diagonal push rods.
#define DELTA_DIAGONAL_ROD diagonal // mm//杆长

// Horizontal offset from middle of printer to smooth rod center.
#define DELTA_SMOOTH_ROD_OFFSET smooth_rod // mm//打印头到竖直铝型材中心的水平距离，用来调节凹凸

// Horizontal offset of the universal joints on the end effector.
#define DELTA_EFFECTOR_OFFSET effector // mm//打印头到杆下端连接处的水平距离

// Horizontal offset of the universal joints on the carriages.
#define DELTA_CARRIAGE_OFFSET carriage // mm//杆上端连接处到竖直铝型材中心的水平距离

// Horizontal distance bridged by diagonal push rods when effector is centered.
#define DELTA_RADIUS max_pos[0]//(DELTA_SMOOTH_ROD_OFFSET-DELTA_EFFECTOR_OFFSET-DELTA_CARRIAGE_OFFSET)//145-26.5-20=98.5
...
#define COS_60 0.5
#define DELTA_TOWER1_X -SIN_60*DELTA_RADIUS // front left tower//-0.866*98.5=-85.3035022727672
#define DELTA_TOWER1_Y -COS_60*DELTA_RADIUS//-0.5*98.5=-49.25
#define DELTA_TOWER2_X SIN_60*DELTA_RADIUS // front right tower
#define DELTA_TOWER2_Y -COS_60*DELTA_RADIUS
#define DELTA_TOWER3_X 0.0 // back middle tower
#define DELTA_TOWER3_Y DELTA_RADIUS//98.5

// Diagonal rod squared
#define DELTA_DIAGONAL_ROD_2 pow(DELTA_DIAGONAL_ROD,2)//求DELTA_DIAGONAL_ROD 219.7的2次方=48268
...
#define TEMP_SENSOR_0 1//temp_sensor_type//   1代表选择    100k的热敏电阻作为传感器                                                                 //1.传感器类别选择
...
#define HEATER_0_MAXTEMP heater_0_maxtemp //275                                                                                      //修改1
//#define HEATER_1_MAXTEMP heater_0_maxtemp
//#define HEATER_2_MAXTEMP 275
#define BED_MAXTEMP bed_maxtemp//150                                                                                                 //修改2
...
设置为190度、循环调用PID_autotune  8次后的相应的PID参数。系统自动生成的PID参数相当不错，可保持默认。
当然机器不同，也不一定都有效
*/
// PID settings:    
...
// Ultimaker
    #define  DEFAULT_Kp Default_Kp//22.2
    #define  DEFAULT_Ki Default_Ki//1.08
    #define  DEFAULT_Kd Default_Kd//144
...
#define X_ENDSTOPS_INVERTING   x_endstops_inverting//1 // set to true to invert the logic of the endstops.                    //修改3
#define Y_ENDSTOPS_INVERTING   y_endstops_inverting//1 // set to true to invert the logic of the endstops.
#define Z_ENDSTOPS_INVERTING   z_endstops_inverting//1 // set to true to invert the logic of the endstops.

#define E0_ENDSTOPS_INVERTING   e0_endstops_inverting//断料检测限位
//#define DISABLE_MAX_ENDSTOPS

#define LEVEL_ENDSTOPS_INVERTING level_endstops_inverting
...
#define INVERT_X_DIR invert_x_dir//false    // for Mendel set to false, for Orca set to true                                      //修改4
#define INVERT_Y_DIR invert_y_dir//true  // for Mendel set to true, for Orca set to false
#define INVERT_Z_DIR invert_z_dir//false    // for Mendel set to false, for Orca set to true
#define INVERT_E0_DIR invert_e0_dir//false  // for direct drive extruder v9 set to true, for geared extruder set to false
#define INVERT_E1_DIR invert_e0_dir    // for direct drive extruder v9 set to true, for geared extruder set to false
...
#define X_MAX_POS 120//110                                                                                                //修改5
#define X_MIN_POS 0//0
#define Y_MAX_POS 120//110
#define Y_MIN_POS 0
...
#define DEFAULT_AXIS_STEPS_PER_UNIT   {480,480,1600,50}  // default steps per unit for ultimaker                         //11.各轴每移动1mm时需要的步数(有细分再乘以细分)
//默认各轴移动1mm时需要的步数(脉冲数)，计费方法参照： http://calculator.josefprusa.cz/                                                                        //修改6
//各轴脉冲数的计算方法：
//X、Y轴：步进电机每转步数(1.8度步距角的电机为200，0.9度步距角的电机为400)*步进电机驱动细分配置(一般16细分)/同步带齿间距(2mm)/同步轮齿数(20)
//Z轴：步进电机每转步数*步进电机驱动细分配置/丝杠导程
//E轴：步进电机每转步数*步进电机驱动细分配置*挤出机齿轮传动比/挤出轮周长2


#define DEFAULT_MAX_FEEDRATE          {10000, 200, 4, 45}    // (mm/sec)                                                          //12.各轴电机的最大速度
//各轴步进电机的速度设置。当你Z轴往上运动比较困难的时候，修改Z轴速度（调小，我这里把默认的5改成2.5了）                                                   //修改7
//配置各电机的最高速度。过高的值需要更大的电流输出，这将导致电机过热，并且有可能使电机在打印时失步。一般可设置为200-400

                                                                                                                                //13.各轴电机的最大加速度
#define DEFAULT_MAX_ACCELERATION      {60000,60000,100,1000}     // X, Y, Z, E maximum start speed for accelerated moves. E default values are good for skeinforge 40+, for older versions raise them a lot.
//该配置为电机最大加速度。过高的电机加速度将导致电机在打印动作时过冲，从而丢失，建议将X\Y最大加速度修改为1000-3000

//默认打印加速度                                                                                                                //14.各轴默认的加速度
#define DEFAULT_ACCELERATION          60000    // X, Y, Z and E max acceleration in mm/s^2 for printing moves
//默认回轴加速度
...
#define DEFAULT_XYJERK                20000.0       // (mm/sec)
...
#define ENABLE_AUTO_BED_LEVELING // Delete the comment to enable (remove // at the start of the line)
  ...
  //这个参数比较重要，尤其Z轴的偏移，即Z探针触发时，打印头与探针的偏移量=打印头到平台的距离=  光电限位一般是负值，机械限位一般是正值
