...
//版本：V1.0
/*版本：V1.1
				修改说明：此fontupd文件虽然名为字体更新，但本质是更新文件，而不仅仅是字体文件，也可是其他文件。
				此1.1版本增加了图标icon_code.bin文件的更新（20150909）
*/
//版权所有，盗版必究。
...
const u8 *THEME_CODE_SDPATH="0:/SYSTEM/THEME/THEME.BIN";	//icon_code.bin的存放位置//
//在25Qxx中的路径
...
//  SPI_Flash_Write((u8*)&themeinfo,THEME_ADDR,sizeof(themeinfo));	//再次写入0xFF， 即使之前是0xAA也不怕都全部重新更新字库  清除之前字库成功的标志.防止更新到一半重启,导致的字库部分数据丢失.
// 	SPI_Flash_Read((u8*)&themeinfo,THEME_ADDR,sizeof(themeinfo));	//重新读出ftinfo结构体数据
	...
	if(tempbuf==NULL)rval=1;
 	...
 	res=f_open(fftemp,(const TCHAR*)theme_code_path,FA_READ); //从SD卡中打开主题文件
...
//		LCD_ShowString(x,y+12,240,320,size,"1"); //20150909	
			themeinfo.themeaddr=THEME_ADDR+sizeof(themeinfo);//信息头之后，
  		themeinfo.themesize=fftemp->fsize;				//UNIGBK大小
		...
		while(res==FR_OK)//死循环执行
		{
			...
			SPI_Flash_Write(tempbuf,offx+flashaddr,4096);		//从0+FONTINFOADDR+sizeof(ftinfo) 开始写入4096个数据
	  		offx+=bread;
			fupd_prog(x+20*size/2,y,size,fftemp->fsize,offx);	 			//进度显示
			if(bread!=4096)break;								//读完了.
	 	}
		f_close(fftemp);
	}
  ...
  SPI_Flash_Write((u8*)&themeinfo,THEME_ADDR,sizeof(themeinfo));	//保存主题信息
	return 0;//无错误.		 
}
...
{	
  ...
  SPI_Flash_Write((u8*)&ftinfo,FONTINFOADDR2,sizeof(ftinfo));	//保存字库信息
...
u8 theme_init(void)
{			  												 
	themeinfo.themeaddr=THEME_ADDR+9;		//UNICODEGBK 表存放首地址固定地
	SPI_Flash_Read((u8*)&themeinfo,THEME_ADDR,sizeof(themeinfo));//读出ftinfo结构体数据   ftinfo虽然是结构体，但是它里面的元素地址也是按地址顺序存储的，正好9个字节
	if(themeinfo.themeok!=0XBB)return 1;		//字库错误.
	return 0;		    
}
