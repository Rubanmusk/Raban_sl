...
//size:字体大小
//gui_phy.language=0 中文简体
//gui_phy.language=1 中文繁体
//gui_phy.language=2 英语
//gui_phy.language=3 韩文
...
{		    
  ...
  if(gui_phy.language==3)//韩文
	{
		if(ql<=0x5A)ql-=0x41;//注意!
		else if(ql<=0x7A)ql-=0x47;
		else ql-=0x4D;
	  qh-=0x81;   
	 foffset=((unsigned long)178*qh+ql)*(size*2);//得到字库中的字节偏移量 
		
	 SPI_Flash_Read(mat,foffset+ftinfo.f12addr,32);	//HANGUL16
//		{
//		  u8 aa[32];
//			SPI_Flash_Read(aa,foffset+ftinfo.f12addr,32);	//HANGUL16
//		}
	}else  	//除韩文外					
	{
